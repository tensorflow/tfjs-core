...
const conv2Weights = dl.variable(
    dl.randomNormal([5, 5, conv2InputDepth, 16], 0, 0.1) as dl.Tensor4D);

const fullyConnectedWeights = dl.variable(
    dl.randomNormal(
        [7 * 7 * conv2OutputDepth, 10], 0,
        1 / Math.sqrt(7 * 7 * conv2OutputDepth)) as dl.Tensor2D);
const fullyConnectedBias = dl.variable(dl.zeros([10]) as dl.Tensor1D);
...
function model(inputXs: dl.Tensor2D): dl.Tensor2D {
  const xs = inputXs.as4D(-1, 28, 28, 1);

  // Conv 1
  const layer1 = dl.tidy(() => {
    return xs.conv2d(conv1Weights, 1, 'same').relu().maxPool([2, 2], 2, 0);
  });

  // Conv 2
  const layer2 = dl.tidy(() => {
    return layer1.conv2d(conv2Weights, 1, 'same').relu().maxPool([2, 2], 2, 0);
  });
