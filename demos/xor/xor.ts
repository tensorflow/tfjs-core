import {Graph, Array1D, NDArrayMathGPU, Session, InCPUMemoryShuffledInputProviderBuilder, SGDOptimizer, CostReduction} from '../deeplearn';
...
const hiddenLayer = graph.layers.dense('hiddenLayer', input, 2, (x) => graph.relu(x), true);
...
for (let i = 0; i < 1000; i += 1) {
  ...
  math.scope(() => {
    ...
    const cost = session.train(costTensor, feedEntries, 5, optimizer, CostReduction.MEAN);
