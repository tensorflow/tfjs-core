...
stridedSlice<T extends Tensor>(
    x: T, begin: number[], end: number[], strides: number[], beginMask: number,
    endMask: number): T {
...
private reduce(
    x: Tensor2D, reduceType: 'max'|'min'|'sum', dtype: DataType): Tensor2D {
...
eluDer<T extends Tensor>(dy: T, y: T): T {
  const program = new BinaryOpProgram(binaryop_gpu.ELU_DER, dy.shape, y.shape);
...
multinomial(
  ...
  const output = this.makeOutputArray(program.outputShape, 'int32') as Tensor2D;
...
private makeOutputArray<T extends Tensor>(shape: number[], dtype: DataType): T {
...
private compileAndRun<T extends Tensor, K extends Tensor>(
  ...
  const outputData = {tensor: output, texData: this.texData.get(output.dataId)};
...
private getAndSaveBinary(
    key: string, getBinary: () => GPGPUBinary): GPGPUBinary {
