...
workers.forEach(worker => {
  worker.onmessage = msg => {
    ...
    workMap.get(msgId).resolve(new Float32Array(data));
  };
});
let nextWorker = 0;
let nextMsgId = 0;
...
export async function depthwiseConv2D(
  ...
  // console.log(
  //     `sendWork x:${xSize} and w:${wVal.length} took`,
  //     (sendWorkTotal).toFixed(0));
  // console.log('sending jobs took', (performance.now() - start).toFixed(0));
