...
describe('step_gpu', () => {
  ...
  it('operates on every element of a matrix', () => {
    ...
    a.fill(0.1);
    const result = uploadStepDownload(a, 4, 6);
  ...
  it('operates on a heterogeneous matrix', () => {
    ...
    const result = uploadStepDownload(a, 4, 1);
    const expected = new Float32Array([0, 0, 1, 0]);
    test_util.expectArraysClose(result, expected, 0);
  });
});

function uploadStepDownload(
    a: Float32Array, rows: number, cols: number): Float32Array {
  return unaryop_gpu.uploadUnaryDownload(a, rows, cols, UnaryOp.STEP);
}
