...
describe('binaryop_gpu Add', () => {
  it('returns a matrix with the same shape as the input matrix', () => {
    ...
    const result = uploadBinaryOpDownload(a, b, binaryop_gpu.ADD);
...
describe('binaryop_gpu Sub', () => {
  ...
  it('2D - 1D invalid shapes for broadcasting', () => {
    ...
    const f = () => uploadBinaryOpDownload(a, b, binaryop_gpu.SUB);
...
describe('binaryop_gpu Divide', () => {
  it('Scalar / Matrix', () => {
    ...
    const r = uploadBinaryOpDownload(c, a, binaryop_gpu.DIV);
...
describe('binaryop_gpu custom', () => {
  it('a ^ 2 + b', () => {
    const c = Scalar.new(2);
    const a = Array2D.new([2, 3], [1, 2, 3, 4, 5, 6]);
    const r = uploadBinaryOpDownload(c, a, 'return a * a + b;');
    expect(r[0]).toBeCloseTo(2 * 2 + 1);
    expect(r[1]).toBeCloseTo(2 * 2 + 2);
    expect(r[2]).toBeCloseTo(2 * 2 + 3);
    expect(r[3]).toBeCloseTo(2 * 2 + 4);
    expect(r[4]).toBeCloseTo(2 * 2 + 5);
    expect(r[5]).toBeCloseTo(2 * 2 + 6);
  });
});

function uploadBinaryOpDownload(
    a: NDArray, b: NDArray, op: string): Float32Array {
