...
const tests: MathTests = it => {
  it('basic', math => {
    ...
    let numTensors = dl.memory().numDataBuffers;
    ...
    expect(dl.memory().numDataBuffers).toBe(numTensors + 1);
    ...
    cost.dispose();
    numTensors = dl.memory().numDataBuffers;

    cost = optimizer.minimize(f, /* returnCost */ false);
    // There should be no new additional Tensors.
    expect(dl.memory().numDataBuffers).toBe(numTensors);
