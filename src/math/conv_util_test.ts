...
describe('conv_util computeConvInfo roundingMode', () => {
  ...
  it('should fail computing the output dimension of Conv Layer', () => {
    const convInfoArgs = [
      inShape, [fSize, fSize, inChannels, chMul], stride, pad, null];

    expect(() => conv_util.computeConv2DInfo.apply(undefined, convInfoArgs))
      .toThrowError();
  });
...
describe('conv_util computePoolInfo roundingMode', () => {
  ...
  it('should fail computing the output dimension of Pool Layer', () => {
    const poolInfoArgs = [
      inShape, [fSize, fSize], stride, pad, null];

    expect(() => conv_util.computePool2DInfo.apply(undefined, poolInfoArgs))
      .toThrowError();
  });
