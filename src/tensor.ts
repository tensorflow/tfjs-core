...
export class Tensor<R extends Rank = Rank> {
  ...
  conv1d<T extends Tensor2D|Tensor3D>(
      this: T, filter: Tensor3D, stride: number, dilation: number,
      pad: 'valid'|'same'|number,
      dimRoundingMode?: 'floor'|'round'|'ceil'): T {
    (this as Tensor).throwIfDisposed();
    return ops.conv1d(this, filter, stride, dilation, pad, dimRoundingMode);
  }
  conv2d<T extends Tensor3D|Tensor4D>(
    ...
    (this as Tensor).throwIfDisposed();
    return ops.conv2d(this, filter, strides, dilations, pad, dimRoundingMode);
  }
