...
export class ArrayOps {
  ...
  static onesLike<T extends Tensor>(x: T|TensorLike): T {
    const $x = util.assertArgIsTensor(x, 'x', 'onesLike');
  ...
  static zerosLike<T extends Tensor>(x: T|TensorLike): T {
    const $x = util.assertArgIsTensor(x, 'x', 'zerosLike');
  ...
  static clone<T extends Tensor>(x: T|TensorLike): T {
    const $x = util.assertArgIsTensor(x, 'x', 'clone');
    const der = (dy: T) => {
      return {$x: () => dy.toFloat()};
    };
  ...
  static multinomial(
    ...
    const $logits = util.assertArgIsTensor(logits, 'logits', 'multinomial');
  ...
  static reshape<R2 extends Rank>(x: Tensor|TensorLike, shape: ShapeMap[R2]):
      Tensor<R2> {
    const $x = util.assertArgIsTensor(x, 'x', 'reshape');
  ...
  static squeeze<T extends Tensor>(x: Tensor|TensorLike, axis?: number[]): T {
    const $x = util.assertArgIsTensor(x, 'x', 'squeeze');
  ...
  static cast<T extends Tensor>(x: T|TensorLike, dtype: DataType): T {
    const $x = util.assertArgIsTensor(x, 'x', 'cast');
  ...
  static tile<T extends Tensor>(x: T|TensorLike, reps: number[]): T {
    const $x = util.assertArgIsTensor(x, 'x', 'tile');
  ...
  static gather<T extends Tensor>(
    ...
    const $x = util.assertArgIsTensor(x, 'x', 'gather');
    const $indices =
        util.assertArgIsTensor(indices, 'indices', 'gather', 'int32');
  ...
  static pad<T extends Tensor>(
      ...
      T {
    const $x = util.assertArgIsTensor(x, 'x', 'pad');
  ...
  static stack<T extends Tensor>(tensors: T[]|TensorLike[], axis = 0): Tensor {
    const $tensors = util.assertArgIsTensorArr(tensors, 'tensors', 'stack');
  ...
  static unstack<T extends Tensor>(x: T|TensorLike, axis = 0): Tensor[] {
    const $x = util.assertArgIsTensor(x, 'x', 'unstack');
  ...
  static split<T extends Tensor>(
    ...
    const $x = util.assertArgIsTensor(x, 'x', 'split');
  ...
  static cumsum<T extends Tensor>(
    ...
    const $x = util.assertArgIsTensor(x, 'x', 'cumsum');
  ...
  static expandDims<R2 extends Rank>(x: Tensor|TensorLike, axis = 0):
      Tensor<R2> {
    const $x = util.assertArgIsTensor(x, 'x', 'expandDims');
