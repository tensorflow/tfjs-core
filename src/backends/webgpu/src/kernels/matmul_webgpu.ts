...
export class MatMulProgram implements WebGPUProgram {
  ...
  constructor(outputShape: [number, number, number]) {
    ...
    this.userCode = `
      ...
      void main() {
        uint localRow = gl_LocalInvocationID.x; // Local row ID (max: TileSize.x)
        uint localCol = gl_LocalInvocationID.y; // Local col ID (max: TileSize.x)
        uint globalRow = TileSize.x*gl_WorkGroupID.x + localRow; // Row ID of C (0..dimAOuter)
        uint globalCol = TileSize.x*gl_WorkGroupID.y + localCol; // Col ID of C (0..dimInner)
